...
<body>
	...
	<table width="1200px;" style="text-align:center; margin:5% auto; border:0; padding:0;">
		<tr>
						...
						<ul style="list-style-type: none;">
							...
							<p>
								<a href="<c:url value='/findCorp/talentDevelopmentCorp.jsp'/>"
									id="talentDevelopmentCorp_anchor">인재 육성형 중소 기업</a>
							</p>
							<p>
								<a href="<c:url value='/findCorp/greenCorp.jsp'/>"
									id="greenCorp_anchor">녹색 기업</a>
							</p>
							<p>
								<a href="<c:url value='/findCorp/socialCorp.jsp'/>"
									id="socialCorp_anchor">사회적 기업</a>
							</p>
							<p>
								<a href="<c:url value='/findCorp/familyFriendlyCorp.jsp'/>"
									id="familyFriendlyCorp_anchor">가족 친화 기업</a>
							</p>
							<p>
								<a href="<c:url value='/findCorp/youthFriendlyCorp.jsp'/>"
									id="youthFriendlyCorp_anchor">청년 친화 강소 기업</a>
							</p>
						</ul>
					</aside>
					<br>
			...
			<td valign="top">
				<div class="content_div_findCorp"
					style="width: 930px; float: right;">
					...
					<div style="margin: 5% auto;">
						...
						<input type="hidden" id="corpType"
							value="${requestScope.corpType }">

						<%-- 뒤로가기 --%>
						<button class="writing_btn" style="background-color: #E7F1FD;"
							onclick="backSpace()">뒤로가기</button>

						<%-- 관심 기업으로 등록하기 --%>
						<%-- 로그인 여부에 따라 버튼 이름 결정 --%>
						<c:choose>
							<c:when
								test="${not empty sessionScope.MemberDTO && requestScope.isRegistered == true}">
								<button class="findCorp_list_btn"
									onclick="addFavoriteCorp_detail(this)">관심기업 삭제</button>
							</c:when>
							<c:otherwise>
								<button class="findCorp_list_btn"
									onclick="addFavoriteCorp_detail(this)">관심기업 등록</button>
							</c:otherwise>
						</c:choose>
					</div>
				</div>
