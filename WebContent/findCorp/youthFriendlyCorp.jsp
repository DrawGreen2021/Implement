...
<title>CorpCollector : 청년 친화 강소 기업</title>
</head>
<body>
	<!-- 헤더 파일 포함 -->
				...
				<ul style="list-style-type: none;">
					...
					<p>
						<a href="<c:url value='/findCorp/youthFriendlyCorp.jsp'/>"
							style="color: #e1bf27; font-weight: bold;">청년 친화 강소 기업</a>
					</p>
				</ul>
			</aside>
			<br>
			...
			<table width="1000px;"
				...
				<tr>
					<table width="900px;"
						...
						<tr>
							<form align="center" action="FindYouthFriendlyCorp.do"
								method="get" name="findYouthFriendlyCorp"
								id="findYouthFriendlyCorp">
				...
				<tr>
					<article class="content_div_findCorp">
						...
						<c:choose>
							...
							<c:when
								test="${requestScope.youthFriendlyCorpList == 'noResult' }">
								검색 결과가 없습니다.
							</c:when>

							<%-- 기업 리스트가 존재하면 출력해주는 테이블 생성 --%>
							<c:when test="${not empty requestScope.youthFriendlyCorpList }">
								...
								<table cellpadding="0" cellspacing="0" border="1">
									...
									<tr>
										...
										<td>업종</td>
										<td>BEST 선정 분야-임금</td>
										<td>BEST 선정 분야-일생활균형</td>
										<td>BEST 선정 분야-고용안정</td>
									</tr>
									<c:forEach items="${requestScope.youthFriendlyCorpList }"
										var="dto">
										<tr>
											<td>☆</td>
								...
								<c:if test="${startNum > 1}">
									<span><a
										href='FindYouthFriendlyCorp.do?page=${startNum - pageCount}&keyword=${param.keyword}'>이전</a>
									</span>
								</c:if>
								<c:if test="${startNum <= 1}">
								...
								<span> <c:forEach var="num" begin="${startNum }"
										...
										<c:if test="${num <= lastPageNum }">
											<a
												href='FindYouthFriendlyCorp.do?page=${num}&keyword=${param.keyword }'>${num}</a>
										</c:if>
									</c:forEach>
								</span>
								...
								<c:if test="${(startNum + pageCount -1) < lastPageNum }">
									<span> <a
										href='FindYouthFriendlyCorp.do?page=${startNum + pageCount}&keyword=${param.keyword}'>다음</a>
									</span>
								</c:if>
								<c:if test="${(startNum + pageCount -1) >= lastPageNum }">
	...
	<script type="text/javascript">
		function resetKeyword() {
			var form = document.findYouthFriendlyCorp;
			var noKeyword = "";

			form.keyword.value = noKeyword;
			form.action = "FindYouthFriendlyCorp.do";
			form.method = "get";
			form.submit();
		}
	</script>
